...
class ApiService {
  ...
  Future<List<dynamic>> getHikes() async {
    try {
      final response = await http.get(Uri.parse('$baseUrl/hikes'));

      if (response.statusCode == 200) {
        final List<dynamic> hikes = jsonDecode(response.body);
        return hikes;
      } else {
        print('Failed to get hikes. Status code: ${response.statusCode}');
        return [];
      }
    } catch (e) {
      print('Error occurred: $e');
